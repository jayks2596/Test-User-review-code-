...
def normalizer(user): ## data cleaning
    ...
    re1 = re.sub(r"(?:\@|http?\://|https?\://|www)\S+", " ", souped) # removing @mentions and the urls
...
def Cross_validation(data, targets, clf_cv, model_name): ## to  Perform cross-validation on support vector classifier 
    ...
    for train_index, test_index in kf.split(data):
        ...
        clf_cv.fit(data_train_cv, targets_train_cv) # SVC model fitting 
...
def c_matrix(max_acc_data_train, max_acc_data_test, max_acc_targets_train, max_acc_targets_test, targets, clf, model_name):
    clf.fit(max_acc_data_train, max_acc_targets_train) #now fitting the classifier
    targets_pred = clf.predict(max_acc_data_test) # Prediction on the testset
...
def main():
    # Reading the training dataset made as a dataframe
    ...
    ## now saving all the cleaned reviews to a different csv
    df.to_csv('Cleaned_userReviews.csv', encoding='utf-8', index=False)
    ##reading the cleaned reviews csv
    ...
    data = tfidf.transform(data) # creating tfidf matrix
    ...
    #### Implementing Oversampling to balance the dataset- SMOTE 
    ...
    SVC_clf = LinearSVC() # creating SVC Model
    ...
    NBC_clf = MultinomialNB() # creating NBC Model
    ...
    adbo= AdaBoostClassifier(random_state=1)
    grid_param = {'n_estimators' : [400]}
    gd_sr = GridSearchCV(estimator=adbo, param_grid=grid_param, scoring='accuracy', cv=10)
...
def main():
    ...
    df['normalized_extract'] = df.extract.apply(normalizer)
    df = df[df['normalized_extract'].map(len) > 0]
    print("Printing top 5 rows of dataframe showing original and cleaned reviews....")
    print(df[['extract','normalized_extract']].head())
    df.drop(['source', 'domain', 'score_max', 'extract', 'product'], axis=1, inplace=True)
    df.to_csv('Cleaned_userReviews.csv', encoding='utf-8', index=False)
